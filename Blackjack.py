...
from GUI import GUI
from Entry_Box import Entry_Box
...
def game(d=0):
    ...
    root = Tk()
    es = Entry_Box(root, user_hand, score(user_hand), dealer_hand[0])
    es.win.mainloop()

    bet = es.get_val()
    print("BET", bet)

    gu = GUI(user_hand, score(user_hand), dealer_hand[0])
...
while(again):
    ...
    if(response == "no"):
        again = False
        root = Tk()
        nameInterface = User_Name(root)
        nameInterface.win.mainloop()

        user_name = nameInterface.get_val()
        storage = Mongo(user_name, d)
